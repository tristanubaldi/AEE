...
## Each Bioclimatic variables are select?
...
count <- "France"
## e.g. multiples countries: count <- c(“Norway”, “Sweden”, “Finland”, “Danemark”)
if (length(count)>1) {
## You have define more than one country, 
## you need to give a name to your countries list.
  count_name <- "Scandinavia"
} else {
  ## If only one country is defined, name is the same.
  count_name <- count }
...
Après avoir définies les données à étudier, la duxième étape est de
télécharger le *Shapefile* du pays d'introduction des espèces de
l'étude, à partir de [gadm.org](gadm.org/index), et le convertir en
*SpatVector* (polygone).
...
`geodata` pour SoilGrids.

Ces rasters, récupérés à l'échelle mondiale, sont ensuite découpés dans
le polygone du (ou des) pays d'introduction, à l'aide de la fonction
`crop()` du package `raster`, afin de ne garder que les conditions
environnementales dans le(s) pays cible.

Les rasters découpés sont ensuite sauvegardés en utilisant la fonction
`writeRaster()` du package `terra`. Les rasters CHELSA sont découpés
pour chaque période et chaque scénario étudié.
...
(**extent**) qui doit être identique à celui des rasters sol (l'étendue
spatial d'un raster représente les coordonnées latitude et longitude
("X, Y") des coins du raster dans l'espace géographique) à l'aide de la
fonction `ext()` du package `terra`.
...
`SpatVector`) de l'espèce à étudier (uniquement sur la période de
référence "1951-1980" car l'aire d'indigénat est calibrée sur la
distribution de l'espèce avant 1980), sauvegardée en tant que `range`.

Dans notre exemple, le Shapefile de l'aire d'indigénat du Robinier a été
téléchargé sur [USTreeAtlas](https://github.com/wpetry/USTreeAtlas),
mais une base de donnée similaire est également disponible pour les
essences natives européennes [EUFORGEN](https://www.euforgen.org/).
...
SpatVector de l'aire d'indigénat ici du Robinier :
...
**Étape 6.1:** Création d'une ACP, qui permet d'obtenir l'espace
...
(bleu) et de l'aire d'indigénat de l'espèce étudiée (jaune; Figure 1a).

**Étape 6.2:** Une fois l'ACP réalisée, les conditions environnementales
au sein de l'aire d'indigénat de l'espèce (en jaune) et dans le pays
d'introduction (en bleu) y sont projetées, donnant deux nuages de
points. Les points concordants, c'est-à-dire les points correspondant
aux conditions environnementales dans le pays d'introduction (en bleu),
ici la France, qui sont à proximité (à une distance maximum (md) de 0.2
unité des axes de l'ACP) des points correspond aux conditions dans
l'aire d'indigénat (la niche réalisée de l'espèce), sont sélectionnés
(ellipse noir, figure 1b).

Cette sélection des points de l'aire d'introduction à proximité des
points de l'aire d'indigénat a été faite à l'aide de la fonction
`distance_semi_join` du package `fuzzyjoin`.

À la fin, un data frame `map_list` avec seulement ces points
sélectionnés permettra de projeter ces points sur les cartes du pays.
...
Exemple d'une ACP réalisée sous huit variables bioclimatiques entre
l'aire d'indigénat du Robinier (jaune) et la France métropolitaine
(bleu) :
...
If variables bioclimatiques null?

-   T -\> Only on soil variables

-   F -\> If period avec scenario?

    -   T -\> Period avec scénario: If aucun point proche sélectionné?

        -   T -\> Créer un data frame vide / plot sans point similaire

        -   F -\> Créer data frame avec points proche / créer plot avec
            points similaires

    -   F -\> Period sans scénario: If aucun point proche sélectionné?
...
sélectionnés (ellipse noir) entre l'aire d'indigénat du Robinier (jaune)
et la France (bleu) :
...
Exemple d'une carte avec la distribution climatique potentielle du
Robinier dans le pays d'introduction (France) sur la période "2071-2100"
sous le scénario SSP5-8.5 :
...
Exemple de la carte avec le total des cartes pour scénarios SSP5-8.5
pour le cas du Robinier (point vert clair la localité géographique sera
bioclimatiquement favorable sous qu'une seule période sous ce scénario;
point vert foncée : la localité géographique sera bioclimatiquement
favorable sous les trois périodes (2010-2040, 2040-2070, 2070-2100) sous
ce scénario).
...
Exemple de la carte avec le total des cartes pour la période "2071-2100"
pour le cas du Robinier (point vert clair : la localité géographique
sera bioclimatiquement favorable sous qu'un seul scénario; point vert
foncée : la localité géographique sera bioclimatiquement favorable sous
les trois scénarios, sur cette période).
...
géographique sera bioclimatiquement favorable sur seulement une seule
...
La première sous-étape est le regroupement des cartes et ACP réalisées
lorsque seules les variables édaphiques sont étudiées :
...
afin d'être informé de sa localisation géographique dans le monde. Ici
avec comme exemple l'aire géographique d'indigénat du Robinier, une
essence originaire des montagnes des Appalaches aux Etats-Unis.
...
patch1 <- (wrap_elements(p1) / wrap_elements(p2) / wrap_elements(p3) / wrap_elements(p4) / wrap_elements(p5)) +
  plot_annotation(
    title = paste("Distribution bioclimatique potentielle de", i, "en", count_name, sep=" "),
    subtitle = sub, 
    theme = theme(plot.title = element_text(size = 30, face="bold", hjust = 0.5),
                  plot.subtitle = element_text(size = 20, hjust = 0.5)))
#patch1
...
Avec ici, comme exemple, la page regroupant l'ensemble des cartes de
distribution climatique potentielle du Robinier en France métropolitaine
entre 1950 et 2100, sous les trois scénarios :

![](images/Carte1.jpg)
Accompagnée de la page avec les ACP réalisées sous huit variables bioclimatiques entre l'aire d'indigénat du Robinier (jaune) et la France métropolitaine (bleu) :

![](images/ACP.jpg) 

Ainsi que la page regroupant la carte de distribution, l'ACP et l'espace environnemental du Robinier obtenus en étudiant les variables édaphiques :
